...
exports.AliExpressAffiliate = void 0;
const AliExpressSDK_1 = require("../../AliExpressSDK");
class AliExpressAffiliate {
    constructor() {
        this.description = {
    ...
    async execute() {
        ...
        try {
            ...
            const defaultTrackingId = credentials.trackingId; // Tracking ID from credentials
            if (!apiKey || !apiSecret) {
                throw new Error('Missing required credential fields');
            }
            const client = new AliExpressSDK_1.IopClient('https://api-sg.aliexpress.com/sync', apiKey, apiSecret);
            for (let i = 0; i < items.length; i++) {
                const operation = this.getNodeParameter('operation', i);
                const request = new AliExpressSDK_1.IopRequest(AliExpressAffiliate.getAliExpressMethod(operation));
                ...
                let trackingId = this.getNodeParameter('trackingId', i, ''); // Default to empty string
                if (!trackingId) {
    ...
    static getAliExpressMethod(operation) {
        const methodMap = {
    ...
    static async addParametersToRequest(executeFunctions, request, operation, itemIndex) {
        switch (operation) {
            case 'searchProducts':
                request.add_api_param('keywords', executeFunctions.getNodeParameter('keywords', itemIndex));
                request.add_api_param('category_ids', executeFunctions.getNodeParameter('category_ids', itemIndex));
                request.add_api_param('min_sale_price', executeFunctions.getNodeParameter('min_sale_price', itemIndex));
                request.add_api_param('max_sale_price', executeFunctions.getNodeParameter('max_sale_price', itemIndex));
                break;
            case 'getProductDetails':
                request.add_api_param('product_ids', executeFunctions.getNodeParameter('product_ids', itemIndex));
                break;
            case 'generateAffiliateLink':
                ...
                request.add_api_param('source_values', executeFunctions.getNodeParameter('source_values', itemIndex));
                // Handle tracking_id either from parameters or credentials
                let trackingId = executeFunctions.getNodeParameter('trackingId', itemIndex, '');
                if (!trackingId) {
                    const credentials = await executeFunctions.getCredentials('aliExpressAffiliateApi');
                    trackingId = credentials.trackingId; // Ensure that the trackingId exists in credentials
                }
                request.add_api_param('tracking_id', trackingId);
                break;
            case 'getHotProducts':
            ...
            case 'getOrderListByIndex':
                request.add_api_param('start_time', executeFunctions.getNodeParameter('start_time', itemIndex));
                request.add_api_param('end_time', executeFunctions.getNodeParameter('end_time', itemIndex));
                request.add_api_param('status', executeFunctions.getNodeParameter('status', itemIndex));
                break;
            case 'getFeaturedPromoProduct':
                request.add_api_param('category_id', executeFunctions.getNodeParameter('category_id', itemIndex));
                request.add_api_param('promotion_name', executeFunctions.getNodeParameter('promotion_name', itemIndex));
                request.add_api_param('sort', executeFunctions.getNodeParameter('sort', itemIndex));
                break;
            case 'getHotProductDownload':
                request.add_api_param('category_id', executeFunctions.getNodeParameter('category_id', itemIndex));
                request.add_api_param('locale_site', executeFunctions.getNodeParameter('locale_site', itemIndex));
                break;
            case 'getOrderInfo':
                request.add_api_param('order_ids', executeFunctions.getNodeParameter('order_ids', itemIndex));
                break;
        }
...
exports.AliExpressAffiliate = AliExpressAffiliate;
